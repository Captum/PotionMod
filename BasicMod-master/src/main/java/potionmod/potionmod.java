...
import potionmod.relics.BaseRelic;
import potionmod.util.GeneralUtils;
import potionmod.util.KeywordInfo;
import potionmod.util.TextureLoader;
import ui.customevents.PotionSellEvent;
...
public class potionmod
        implements EditRelicsSubscriber, EditStringsSubscriber, EditKeywordsSubscriber, PostInitializeSubscriber {
    ...
    public void receivePostInitialize() {
        ...
        BaseMod.addEvent(PotionSellEvent.ID, PotionSellEvent.class, Exordium.ID);
    ...
    public void receiveEditStrings() {
        /*
         * First, load the default localization. Then, if the current language is
         * different, attempt to load localization for that language. This results in
         * the default localization being used for anything that might be missing. The
         * same process is used to load keywords slightly below.
         */
    ...
    private static void loadModInfo() {
        Optional<ModInfo> infos = Arrays.stream(Loader.MODINFOS).filter((modInfo) -> {
            ...
            return initializers.contains(potionmod.class.getName());
        }).findFirst();
        if (infos.isPresent()) {
            info = infos.get();
            modID = info.ID;
